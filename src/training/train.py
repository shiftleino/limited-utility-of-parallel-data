...
from olmo.exceptions import OLMoConfigurationError, OLMoError
...
def main(cfg: TrainConfig) -> None:
    ...
    if torch.cuda.is_available():
        ...
        raise OLMoError("CUDA is not available. Please use a GPU.")
    ...
    log.info(f"Using {cfg.data.num_workers} data workers for training data loader.")
    ...
    if cfg.distributed_strategy == DistributedStrategy.ddp:
        ...
        # Set up device mesh for hybrid sharding in order to specify which nodes are associated to a given model replica
        device_mesh = None
    ...
    with Trainer(
        ...
        if not cfg.dry_run and not cfg.no_pre_train_checkpoint and cfg.load_path is None:
            ...
            # And then we verify that we can load it.
        ...
        if cfg.load_path is not None:
            ...
            if cfg.reset_optimizer_state and not cfg.reset_trainer_state:
                raise OLMoConfigurationError(
                    "You set reset_optimizer_state=True and not reset_trainer_state=True. "
                    "This will reset the optimizer but keep the trainer state, which could "
                    "create inconsistencies. Either set reset_trainer_state=True as well, "
                    "or set reset_optimizer_state=False."
                )
...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        format='%(asctime)s - RANK:%(levelname)s - %(name)s - %(process)d - %(message)s'
    )
    try:
    ...
    parser.add_argument("--train-config-path", type=str, help="The path for the TrainConfig.yaml-file")
    args = parser.parse_args()

    yaml_path = args.train_config_path
    cfg = TrainConfig.load(yaml_path)

    if not torch.cuda.is_available():
        raise OLMoError("CUDA is not available. Please use a GPU.")
    
    # When training with LUMI, set CPU affinity
    if cfg.lumi_env:
        log.info(f"Using LUMI for training, setting CPU affinity")
        set_cpu_affinity(get_local_rank())
