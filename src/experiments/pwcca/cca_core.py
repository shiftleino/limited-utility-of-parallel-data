...
def get_cca_similarity(acts1, acts2, epsilon=0., threshold=0.98,
  ...
  if (not np.any(x_idxs)) or (not np.any(y_idxs)):
    return_dict = {
        "x_idxs": x_idxs,
        "y_idxs": y_idxs,
        "cca_coef1": np.array([0.0]),
        "cca_coef2": np.array([0.0]),
    ...
    # Add placeholders for coefficient keys needed by pwcca to avoid KeyErrors
    if compute_coefs:
        numx = acts1.shape[0]
        numy = acts2.shape[0]
        return_dict["coef_x"] = np.zeros((numx, numx))
        return_dict["coef_y"] = np.zeros((numy, numy))

    if compute_dirns:
        return_dict["cca_dirns1"] = np.zeros_like(acts1)
        return_dict["cca_dirns2"] = np.zeros_like(acts2)
