...
from olmo.checkpoint import Checkpointer, FullCheckpointer, build_sharded_checkpointer
...
from olmo.eval import Evaluator
from olmo.exceptions import OLMoConfigurationError
from olmo.model import OLMo
from olmo.optim import Optimizer, Scheduler
...
class SpeedMonitor:
    ...
    def batch_start(
        ...
        # num_fwd_flops and num_bck_flops from the model computes flops per token
...
try:
    ...
    log.info("Flash attention import successful")
...
class Trainer:
    ...
    def tokens_per_epoch(self) -> int:
        return self.dataset.total_size * self.cfg.model.max_sequence_length
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        if not self.cfg.restore_dataloader:
            ...
            if self.cfg.reset_dataloader_epoch_progress_on_load:
                log.info(
                    f"Global step is {self.global_step}. "
                    f"`reset_dataloader_epoch_progress_on_load` is True. "
                    f"Resetting dataloader epoch progress for epoch {self.epoch}. "
                )
                self.global_train_examples_seen_this_epoch = 0
    ...
    def train_micro_batch(
        self, micro_batch: Dict[str, Any], valid_batch_size_in_tokens: int
    ) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
        ce_loss, z_loss, logits = self.model_forward(
            micro_batch, compute_z_loss=self.cfg.softmax_auxiliary_loss, loss_reduction="sum"
        )
        ce_loss = ce_loss / valid_batch_size_in_tokens
        ...
        if self.cfg.softmax_auxiliary_loss:
            ...
            z_loss = z_loss / valid_batch_size_in_tokens
            loss = ce_loss + z_loss
        else:
            loss = ce_loss
    ...
    def train_batch(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        # Calculate number of valid tokens for scaling the gradients accurately if examples are padded/masked
        valid_batch_size_in_tokens = (batch["labels"] != -100).sum()
        ...
        for micro_batch_idx, micro_batch in enumerate(micro_batches):
            ...
            with grad_sync_context():
                ...
                with torch.autocast(autocast_device, enabled=True, dtype=self.cfg.autocast_precision):
                    # Run forward pass.
                    loss, ce_loss, z_loss = self.train_micro_batch(micro_batch, valid_batch_size_in_tokens)
    ...
    def fit(self):
        ...
        if self.cfg.torch_profiling and get_global_rank() == 0:
            ...
            def on_trace_ready(p):
                ...
                if self.cfg.remote_save_folder is not None:
                    raise NotImplementedError("Haven't implemented remote folder access")
        ...
        unsharded_checkpoints = set(self.cfg.save_steps_unsharded) if self.cfg.save_steps_unsharded is not None else set()
        
        log.info(f"Total number of tokens per epoch: {self.tokens_per_epoch}")
        log.info(f"Tokens covered now: {self.max_tokens} | {self.max_tokens/self.tokens_per_epoch*100:.2f}%")
            ...
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    if (
                        ...
                        or 
                        (save_checkpoints
                         and unsharded_checkpoints is not None
                         and self.global_step in unsharded_checkpoints
                         and self.cfg.save_num_unsharded_checkpoints_to_keep != 0)
                    ):
        ...
        if save_checkpoints:
            if (
                (self.cfg.save_interval_unsharded is not None or unsharded_checkpoints is not None)
