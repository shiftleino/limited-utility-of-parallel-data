...
from olmo.torch_util import barrier, get_fs_local_rank, get_global_rank, get_world_size
from olmo.util import roundrobin, threaded_generator
...
class IterableDataset(torch.utils.data.IterableDataset[Dict[str, Any]]):
    ...
    def __init__(
        ...
        *,
        ...
        if work_dir is not None:
            ...
            self._manage_global_indices_file_creation()
        else:
            self.global_indices_file = None
    
    def _manage_global_indices_file_creation(self):
        if self.rank == 0:
            self.work_dir.mkdir(parents=True, exist_ok=True)

            if self.global_indices_file.exists() and not self.save_overwrite:
                log.info(f"[Rank {self.rank}] Using existing global indices file: "
                        f"{self.global_indices_file} (save_overwrite=False).")
            else:
                if self.global_indices_file.exists():
                    log.info(f"[Rank {self.rank}] Overwriting existing global indices file: "
                            f"{self.global_indices_file} (save_overwrite=True).")
                else:
                    log.info(f"[Rank {self.rank}] Building and saving global data order indices to "
                            f"{self.global_indices_file}")
                
                # Use a unique temp file name, e.g., including epoch and PID
                temp_file = self.global_indices_file.with_suffix(f".epoch{self.epoch}.pid{os.getpid()}.tmp")
                try:
                    global_indices_arr = self._build_global_indices() 
                    
                    log.info(f"[Rank {self.rank}] Saving {global_indices_arr.size} indices "
                            f"(dtype: {global_indices_arr.dtype}, shape: {global_indices_arr.shape}) "
                            f"to temp file {temp_file} for {self.global_indices_file}.")

                    memmap_obj = np.memmap(
                        temp_file, dtype=np.uint32, mode="w+", shape=(len(global_indices_arr),)
                    )
                    memmap_obj[:] = global_indices_arr
                    memmap_obj.flush()
                    del memmap_obj
                    
                    temp_file.rename(self.global_indices_file) # Atomic rename
                    log.info(f"[Rank {self.rank}] Global indices for epoch {self.epoch} saved to '{self.global_indices_file}'")
                except Exception as e:
                    log.error(f"[Rank {self.rank}] CRITICAL: Rank 0 failed to save global indices to "
                            f"{self.global_indices_file} (via {temp_file}): {e}", exc_info=True)
                    if temp_file.exists():
                        try: temp_file.unlink(missing_ok=True)
                        except OSError as unlink_e: log.error(f"[Rank {self.rank}] Failed to unlink temp file {temp_file}: {unlink_e}")
                    raise

        log.debug(f"[Rank {self.rank}] Waiting at barrier after rank 0 file management for epoch {self.epoch}.")
        barrier() 
        if not self.global_indices_file.exists():
            err_msg = (f"[Rank {self.rank}] CRITICAL: Global indices file {self.global_indices_file} "
                    f"not found after barrier. Rank 0 must have failed to create it.")
            log.error(err_msg)
            raise FileNotFoundError(err_msg)
        log.info(f"[Rank {self.rank}] Confirmed global indices file is ready: {self.global_indices_file}")
    ...
    def reshuffle_old(self, epoch: int):
        self.epoch = epoch
        if self.work_dir is not None:
            self._build_and_save_global_indices()
    
    def reshuffle(self, epoch: int):
        log.info(f"[Rank {self.rank}] Reshuffling dataset for epoch {epoch}.")
        self.epoch = epoch # YOUR _build_global_indices will use this

        if self.work_dir is not None:
            if self.rank == 0:
                log.info(f"[Rank {self.rank}] Leader (rank 0) is rebuilding and saving indices for new epoch {self.epoch}.")
                temp_file = self.global_indices_file.with_suffix(f".epoch{self.epoch}.pid{os.getpid()}.tmp")
                try:
                    global_indices_arr = self._build_global_indices()
                    
                    log.info(f"[Rank {self.rank}] Reshuffle: Saving {global_indices_arr.size} indices to temp {temp_file}.")             
                    memmap_obj = np.memmap(
                        temp_file, dtype=self.expected_dtype, mode="w+", shape=global_indices_arr.shape
                    )
                    memmap_obj[:] = global_indices_arr
                    memmap_obj.flush()
                    del memmap_obj
                    
                    temp_file.rename(self.global_indices_file)
                    log.info(f"[Rank {self.rank}] Reshuffle: Global indices for epoch {self.epoch} saved to '{self.global_indices_file}'")
                except Exception as e:
                    log.error(f"[Rank {self.rank}] CRITICAL: Rank 0 failed during reshuffle's save for epoch {self.epoch}: {e}", exc_info=True)
                    if temp_file.exists():
                        try: temp_file.unlink(missing_ok=True)
                        except OSError as unlink_e: log.error(f"[Rank {self.rank}] Failed to unlink temp file {temp_file}: {unlink_e}")
                    raise
            
            log.debug(f"[Rank {self.rank}] Waiting at barrier after rank 0 reshuffle for epoch {self.epoch}.")
            barrier()

            if not self.global_indices_file.exists():
                raise FileNotFoundError(
                    f"[Rank {self.rank}] CRITICAL: Indices file {self.global_indices_file} missing after reshuffle for epoch {self.epoch}."
                )
            log.info(f"[Rank {self.rank}] Confirmed indices file ready after reshuffle for epoch {self.epoch}.")
        else:
            log.info(f"[Rank {self.rank}] Reshuffling (no work_dir). Indices will be rebuilt in memory by _build_global_indices via get_global_indices.")
