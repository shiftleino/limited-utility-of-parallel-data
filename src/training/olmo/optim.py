...
from olmo.model import LayerNorm
from olmo.config import SchedulerConfig, TrainConfig, SchedulerType
from olmo.torch_util import get_default_device, is_distributed
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith("bias"):
                ...
                decay.add(fpn)
            elif pn.endswith("weight") and isinstance(m, (LayerNorm, nn.LayerNorm)):
                if cfg.optimizer.decay_norm_and_bias:
                    decay.add(fpn)
                else:
                    no_decay.add(fpn)
...
def build_scheduler(cfg: TrainConfig, sched_cfg: Optional[SchedulerConfig] = None) -> Scheduler:
    ...
    if not sched_cfg.name or sched_cfg.name == SchedulerType.cosine_with_warmup:
